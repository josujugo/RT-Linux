...
static void initC(python_block *block)
{
  ...
  block->intPar[0] = sock;
  block->intPar[1] = ret;
  pthread_create(&thrd, NULL, reest_connection1, (void *) block);
}
...
static void inoutC(python_block *block)
{
  ...
  if(block->intPar[0]){
    ...
    ret = recv(block->intPar[0], &values, sizeof(values),0);
...
void unixsockC_S(int flag, python_block *block)
{
...
void * est_connection1(void * p)
{
  ...
  while(1){
    ...
    fd[0].fd = sc; // your socket handler 
    fd[0].events = POLLIN;
    ...
    while(sc){
      ret1 = poll(fd, 1, 1000); // 1 second for timeout
      switch(ret1){
            case 0: //POLL_EXPIRE:
                printf("Timeout \n");
                sc=FALSE;
...
static void initS(python_block *block)
{
  ...
  pthread_t thrd;
  pthread_create(&thrd, NULL, est_connection1, (void *) block);
...
void unixsockS_C(int flag, python_block *block)
{
  if (flag==CG_OUT){          /* get input */
    inoutS(block);
  }
